...
# 建立模型
model = Sequential([
    Conv2D(48, (3, 3), activation='relu', input_shape=(224, 224, 3)),  # 從32增加到48濾波器
    BatchNormalization(),  # 新增批量標準化層
    MaxPooling2D(2, 2),
...
train_generator = train_datagen.flow_from_dataframe(
    ...
    target_size=(224, 224),
    class_mode='binary',
    batch_size=1  # 為了適應計算資源限制，保持批次大小為1
)
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.xticks(range(0, 11))  # 確保x軸顯示整數epoch
plt.legend()
